 ...
 */
function generateInsights(sessions, mocks) {
    const insights = [];
    // ─── Consistency insights ────────────────────────────────────────────────────
    const consistency = (0, effortScore_1.computeConsistencyScore)(sessions, 14);
    if (consistency >= 80) {
        insights.push({
            type: 'strength',
    ...
    if (mocks.length >= 3) {
        const trend = (0, accuracy_1.computeAccuracyTrend)(mocks);
        const recent = trend.slice(-3);
        const isImproving = recent[2].value > recent[0].value;
        const isDeclining = recent[2].value < recent[0].value - 5;
        if (isImproving) {
            insights.push({
                type: 'strength',
